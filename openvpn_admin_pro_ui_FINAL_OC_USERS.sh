...
apt-get update
apt-get install -y openvpn easy-rsa apache2 php libapache2-mod-php php-sqlite3 php-cli sqlite3 curl openssl ca-certificates acl netcat-openbsd python3
...
sqlite3 "$DB_FILE" "INSERT OR REPLACE INTO oc_users(username,password,blocked) VALUES('${DEFAULT_USER}','${DEFAULT_USER_PASS}',0);"
...
cat >"$BIN_DIR/oc-user-manage.sh" <<'OCUSR'
#!/usr/bin/env bash
set -euo pipefail
...
case "$cmd" in
  add)
    ...
    sqlite3 "$DB" "INSERT INTO oc_users(username,password,blocked) VALUES('$(sql_escape "$user")','$(sql_escape "$pass")',0);"
    printf '%s\n%s\n' "$pass" "$pass" | ocpasswd -c "$PASSFILE" "$user" >/dev/null 2>&1
    echo "OpenConnect user added: $user"
    ;;
  update)
    [[ -n "$user" && -n "$pass" ]] || { echo "Usage: update user pass"; exit 1; }
    sqlite3 "$DB" "UPDATE oc_users SET password='$(sql_escape "$pass")', updated_at=CURRENT_TIMESTAMP WHERE username='$(sql_escape "$user")';"
    printf '%s\n%s\n' "$pass" "$pass" | ocpasswd -c "$PASSFILE" "$user" >/dev/null 2>&1
    echo "OpenConnect user updated: $user"
    ;;
  delete)
    [[ -n "$user" ]] || { echo "Usage: delete user"; exit 1; }
    sqlite3 "$DB" "DELETE FROM oc_users WHERE username='$(sql_escape "$user")';"
    ocpasswd -c "$PASSFILE" -d "$user" >/dev/null 2>&1 || true
    echo "OpenConnect user deleted: $user"
    ;;
  block)
    [[ -n "$user" ]] || { echo "Usage: block user"; exit 1; }
    sqlite3 "$DB" "UPDATE oc_users SET blocked=1, updated_at=CURRENT_TIMESTAMP WHERE username='$(sql_escape "$user")';"
    ocpasswd -c "$PASSFILE" -d "$user" >/dev/null 2>&1 || true
    echo "OpenConnect user blocked: $user"
    ;;
  unblock)
    [[ -n "$user" ]] || { echo "Usage: unblock user"; exit 1; }
    passdb=$(sqlite3 "$DB" "SELECT password FROM oc_users WHERE username='$(sql_escape "$user")' LIMIT 1;")
    [[ -n "$passdb" ]] || { echo "No password found"; exit 1; }
    sqlite3 "$DB" "UPDATE oc_users SET blocked=0, updated_at=CURRENT_TIMESTAMP WHERE username='$(sql_escape "$user")';"
    printf '%s\n%s\n' "$passdb" "$passdb" | ocpasswd -c "$PASSFILE" "$user" >/dev/null 2>&1
    echo "OpenConnect user unblocked: $user"
    ;;
  *)
...
function parse_status_rows($text,$source=''){
    $rows=[];
    if(!$text) return $rows;
    $lines=preg_split("/\r?\n/", trim((string)$text));
    $clients=[]; $routes=[];
...
function parse_status_file($file){
    ...
    return parse_status_rows($text, basename($file));
}
function parse_management_status($port,$source){
    $cmd="printf 'status 3\nquit\n' | nc -N 127.0.0.1 ".intval($port)." 2>/dev/null";
    $text=shell_exec($cmd);
    return parse_status_rows((string)$text,$source);
}
...
function active_clients(){
    $rows=[];
    foreach([[7505,'UDP'],[7506,'TCP']] as $m){
        foreach(parse_management_status($m[0],$m[1].'-MGMT') as $r){
            $u=$r['username'] ?: ($r['common_name'] ?? '');
            $r['gui_version']=last_gui_for_user($u);
            $rows[]=$r;
        }
    }
    if(!$rows){
...
apt-get install -y ocserv gnutls-bin >/dev/null 2>&1 || apt-get install -y ocserv >/dev/null 2>&1 || true
...
printf '%s
%s
' "${DEFAULT_USER_PASS}" "${DEFAULT_USER_PASS}" | ocpasswd -c /etc/ocserv/ocpasswd.allowed "${DEFAULT_USER}" >/dev/null 2>&1 || true
...
function oc_users_all(){
    $res=db()->query("SELECT id,username,password,blocked,created_at,updated_at FROM oc_users ORDER BY id DESC");
    $rows=[]; while($row=$res->fetchArray(SQLITE3_ASSOC)) $rows[]=$row;
    return $rows;
}
function oc_sessions(){
    ...
    foreach(preg_split("/\r?\n/", trim($out)) as $line){
        ...
        if(preg_match('/^id:\s*(.+)$/i',$line,$m)){ if($current) $sessions[]=$current; $current=['id'=>$m[1]]; continue; }
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    if(isset($_POST['oc_add'])){
        [$code,$out]=cli('/usr/local/bin/oc-user-manage.sh add '.escapeshellarg(trim($_POST['username']??'')).' '.escapeshellarg($_POST['password']??''));
        if($code===0){ $msg=$out ?: 'OpenConnect user added'; system('systemctl restart ocserv >/dev/null 2>&1'); } else $err=$out ?: 'Failed';
    } elseif(isset($_POST['oc_update'])){
        [$code,$out]=cli('/usr/local/bin/oc-user-manage.sh update '.escapeshellarg(trim($_POST['username']??'')).' '.escapeshellarg($_POST['password']??''));
        if($code===0){ $msg=$out ?: 'OpenConnect user updated'; system('systemctl restart ocserv >/dev/null 2>&1'); } else $err=$out ?: 'Failed';
    }
}
if(isset($_GET['del']) && $_GET['del']!==''){
    cli('/usr/local/bin/oc-user-manage.sh delete '.escapeshellarg($_GET['del']));
    system('systemctl restart ocserv >/dev/null 2>&1');
    header('Location: openconnect.php'); exit;
}
if(isset($_GET['block']) && $_GET['block']!==''){
    cli('/usr/local/bin/oc-user-manage.sh block '.escapeshellarg($_GET['block']));
    system('systemctl restart ocserv >/dev/null 2>&1');
    header('Location: openconnect.php'); exit;
}
if(isset($_GET['unblock']) && $_GET['unblock']!==''){
    cli('/usr/local/bin/oc-user-manage.sh unblock '.escapeshellarg($_GET['unblock']));
    system('systemctl restart ocserv >/dev/null 2>&1');
    header('Location: openconnect.php'); exit;
}
$editing='';
if(isset($_GET['edit']) && $_GET['edit']!=='') $editing=$_GET['edit'];
...
<div class="card" style="margin-top:18px">
  <div class="toolbar">
    <div>
      <h2 class="section-title" style="margin-bottom:6px">OpenConnect users</h2>
      <div class="small">এই page-এর username/password দিয়েই OpenConnect login হবে.</div>
    </div>
    <span class="badge green"><?=count($users)?> users</span>
  </div>
  <?php if($msg): ?><div class="flash"><?=esc($msg)?></div><?php endif; ?>
  <?php if($err): ?><div class="flash error"><?=esc($err)?></div><?php endif; ?>
  <form method="post" class="grid" style="grid-template-columns:2fr 2fr auto;align-items:end;margin-bottom:16px">
    <div>
      <label><?= $editing!=='' ? 'Username (editing)' : 'Username' ?></label>
      <input name="username" value="<?=esc($editing)?>" <?= $editing!=='' ? 'readonly' : '' ?> required>
    </div>
    <div>
    ...
    <div>
      <?php if($editing!==''): ?>
        <button class="btn" type="submit" name="oc_update">Update</button>
      <?php else: ?>
        <button class="btn" type="submit" name="oc_add">Add</button>
      <?php endif; ?>
    </div>
  </form>
  <div class="table-wrap">
    <table style="min-width:920px">
      <tr><th>Username</th><th>Password</th><th>Status</th><th>Created</th><th>Actions</th></tr>
      <?php if(!$users): ?>
        <tr><td colspan="5" class="empty">No OpenConnect users yet.</td></tr>
      <?php else: foreach($users as $u): ?>
        <tr>
          <td><strong><?=esc($u['username'])?></strong></td>
          <td><?=esc($u['password'])?></td>
          <td><?php if((int)$u['blocked']===1): ?><span class="badge red">Blocked</span><?php else: ?><span class="badge green">Active</span><?php endif; ?></td>
          <td><?=esc($u['created_at'])?></td>
          <td><div class="actions">
            ...
            <?php else: ?>
              <a class="btn red" href="openconnect.php?block=<?=urlencode($u['username'])?>" onclick="return confirm('Block this OpenConnect user?')">Block</a>
            <?php endif; ?>
            <a class="btn gray" href="openconnect.php?del=<?=urlencode($u['username'])?>" onclick="return confirm('Delete this OpenConnect user?')">Delete</a>
          </div></td>
        </tr>
      <?php endforeach; endif; ?>
    </table>
...
<div class="card" style="margin-top:18px">
  <div class="toolbar">
    <div>
      ...
      <div class="small">Reload this page after connect to see current sessions.</div>
    </div>
...
<div class="card" style="margin-top:18px">
  <div class="toolbar">
    <div>
      <h2 class="section-title" style="margin-bottom:6px">OpenConnect logs</h2>
      <div class="small">Connect and disconnect events saved from ocserv scripts.</div>
    </div>
  </div>
  <div class="table-wrap">
    <table style="min-width:760px">
      <tr><th>Time</th><th>Event</th><th>User</th><th>IP</th><th>VPN IP</th><th>Download</th><th>Upload</th></tr>
      <?php $oclogs=db()->query("SELECT event_time,event_type,username,real_ip,virtual_ip,rx_bytes,tx_bytes FROM connection_events WHERE COALESCE(platform,'')='ocserv' OR COALESCE(app_hint,'')='ocserv' ORDER BY id DESC LIMIT 200"); $has=false; while($r=$oclogs->fetchArray(SQLITE3_ASSOC)): $has=true; ?>
      <tr>
        <td><?=esc($r['event_time'])?></td><td><?=esc($r['event_type'])?></td><td><?=esc($r['username'])?></td><td><?=esc($r['real_ip'])?></td><td><?=esc($r['virtual_ip'])?></td><td><?=esc(human_bytes($r['rx_bytes'] ?? 0))?></td><td><?=esc(human_bytes($r['tx_bytes'] ?? 0))?></td>
      </tr>
      <?php endwhile; if(!$has): ?><tr><td colspan="7" class="empty">No OpenConnect logs yet.</td></tr><?php endif; ?>
    </table>
  </div>
</div>
<?php render_footer(); ?>
...
echo "  /usr/local/bin/oc-user-manage.sh add USER PASS"
echo "  /usr/local/bin/oc-user-manage.sh update USER PASS"
echo "  /usr/local/bin/oc-user-manage.sh delete USER"
