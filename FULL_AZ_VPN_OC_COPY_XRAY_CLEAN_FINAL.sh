...
function copyOpenConnectUrl(){
  ...
  if(navigator.clipboard){
    navigator.clipboard.writeText(val).then(function(){ alert('Copied: ' + val); }).catch(function(){
      el.select(); document.execCommand('copy'); alert('Copied: ' + val);
    });
  } else {
...
$active = trim(shell_exec("ss -tn state established '( sport = :4443 or dport = :4443 )' 2>/dev/null | tail -n +2 | wc -l"));
if($active === '') $active = '0';
...
<div class="grid">
  <div class="card">
    <div class="muted">Xray active connections</div>
...
function copyXrayLink(){
  ...
  if(navigator.clipboard){
    navigator.clipboard.writeText(val).then(function(){ alert('Copied Xray config'); }).catch(function(){
      el.select(); document.execCommand('copy'); alert('Copied Xray config');
    });
  } else {
