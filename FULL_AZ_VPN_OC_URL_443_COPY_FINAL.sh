...
$serverIp = trim(shell_exec("curl -4 -fsSL https://api.ipify.org 2>/dev/null")) ?: ($_SERVER['SERVER_ADDR'] ?? $_SERVER['HTTP_HOST'] ?? 'SERVER_IP');
...
<div class="card" style="margin-top:18px">
  <div class="toolbar">
    <div>
      ...
      <div class="small">এই URL দিয়ে OpenConnect connect হবে। Port updated: 443</div>
    </div>
    <span class="badge green">443</span>
  </div>
  <div class="copy-row" style="display:flex;gap:10px;align-items:center;margin-top:12px">
    <input id="ocUrlInput" value="<?=esc($ocUrl)?>" readonly style="flex:1;min-width:0">
    <button class="btn green" type="button" onclick="copyOCUrl()">Copy</button>
  </div>
</div>
