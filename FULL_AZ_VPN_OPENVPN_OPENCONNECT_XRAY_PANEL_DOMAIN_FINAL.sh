...
backend ocserv_backend
    mode tcp
    server ocserv 127.0.0.1:4443 check

backend xray_backend
    mode tcp
    server xray 127.0.0.1:8444 check
EOF
...
function xray_active_connections(){
    $count = xcmd("ss -tn sport = :8444 | grep ESTAB | wc -l");
    return (int)$count;
}
...
$link = $uuid ? "vless://".$uuid."@".$v2host.":443?type=tcp&security=none#Xray-".$v2host : '';
...
echo "  UUID: ${XRAY_UUID}"
echo "  Link: vless://${XRAY_UUID}@${V2_HOST}:443?type=tcp&security=none#Xray-${V2_HOST}"
