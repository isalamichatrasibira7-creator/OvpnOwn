...
function users_all(){
    $sql = "SELECT u.id,u.username,u.blocked,u.created_at,u.updated_at,
                   COALESCE(SUM(CASE WHEN e.event_type='disconnect' THEN e.rx_bytes ELSE 0 END),0) AS total_rx,
                   COALESCE(SUM(CASE WHEN e.event_type='disconnect' THEN e.tx_bytes ELSE 0 END),0) AS total_tx,
                   MAX(e.event_time) AS last_seen,
                   MAX(CASE WHEN COALESCE(e.gui_version,'')<>'' THEN e.gui_version ELSE '' END) AS latest_gui,
                   MAX(CASE WHEN COALESCE(e.real_ip,'')<>'' THEN e.real_ip ELSE '' END) AS latest_ip
            FROM users u
            LEFT JOIN connection_events e ON e.username=u.username
            GROUP BY u.id
            ORDER BY u.id DESC";
    $res=db()->query($sql);
...
function latest_logs($limit=300,$search=''){
    if($search!==''){
        $st=db()->prepare('SELECT * FROM connection_events WHERE username LIKE :s OR real_ip LIKE :s OR gui_version LIKE :s OR app_hint LIKE :s ORDER BY id DESC LIMIT :l');
        $st->bindValue(':s','%'.$search.'%',SQLITE3_TEXT);
        $st->bindValue(':l',(int)$limit,SQLITE3_INTEGER);
        $r=$st->execute();
    } else {
        $st=db()->prepare('SELECT * FROM connection_events ORDER BY id DESC LIMIT :l');
        $st->bindValue(':l',(int)$limit,SQLITE3_INTEGER);
        $r=$st->execute();
    }
...
<div class="card" style="margin-top:18px">
  <div class="toolbar">
    <div>
      ...
      <div class="small">Refresh the page to update the active count and live device table.</div>
    </div>
  ...
  <div class="table-wrap">
    <table>
      <tr>
        <th>User</th><th>Common Name</th><th>Real Address</th><th>Virtual IP</th><th>Since</th><th>Download</th><th>Upload</th><th>Proto</th><th>Action</th>
      </tr>
      <?php if(!$active): ?>
        <tr><td colspan="9" class="empty">No active devices right now. Connect a device and refresh after a few seconds.</td></tr>
      <?php else: foreach($active as $c): $u=($c['username'] ?: $c['common_name']); ?>
        <tr>
          <td class="row-stack"><strong><?=esc($u)?></strong><span class="small"><?=esc(last_gui_for_user($u))?></span></td>
          <td><?=esc($c['common_name'])?></td>
          <td><?=esc($c['real_address'])?></td>
          <td><?=esc($c['virtual_address'])?></td>
          <td><?=esc($c['connected_since'])?></td>
          <td><?=esc(human_bytes($c['bytes_received']))?></td>
          <td><?=esc(human_bytes($c['bytes_sent']))?></td>
          <td><?=esc($c['source'])?></td>
          <td><a class="btn red" href="block_user.php?u=<?=urlencode($u)?>" onclick="return confirm('Block this user? Active sessions may reconnect-fail after block.')">Block</a></td>
        </tr>
...
<?php require __DIR__.'/config.php'; require_login(); $users=users_all(); $activeMap=active_clients_by_user(); render_header('Users'); ?>
<div class="card">
  <div class="toolbar">
    <div>
      <h2 class="section-title" style="margin-bottom:6px">All users</h2>
      <div class="small">Cleaner list with block status, usage totals, latest IP and latest GUI.</div>
    </div>
  ...
  <div class="table-wrap">
    <table>
      <tr>
        <th>User</th><th>Status</th><th>Active Devices</th><th>Total Download</th><th>Total Upload</th><th>Latest IP</th><th>Latest GUI</th><th>Created</th><th>Actions</th>
      </tr>
      <?php if(!$users): ?>
        <tr><td colspan="9" class="empty">No users found.</td></tr>
      <?php else: foreach($users as $u): $count=count($activeMap[$u['username']] ?? []); ?>
        <tr>
          <td class="row-stack"><strong><?=esc($u['username'])?></strong><span class="small">Updated: <?=esc($u['updated_at'])?></span></td>
          <td>
            <?php if((int)$u['blocked']===1): ?><span class="badge red">Blocked</span>
            <?php else: ?><span class="badge green">Allowed</span><?php endif; ?>
          </td>
          <td><span class="badge <?=$count>0 ? 'green' : ''?>"><?=$count?> device<?= $count===1 ? '' : 's' ?></span></td>
          <td><?=esc(human_bytes($u['total_rx']))?></td>
          <td><?=esc(human_bytes($u['total_tx']))?></td>
          <td><?=esc($u['latest_ip'] ?: '-')?></td>
          <td class="small"><?=esc($u['latest_gui'] ?: '-')?></td>
          ...
          <td>
            <div class="actions">
              ...
              <?php if((int)$u['blocked']===1): ?>
                <a class="btn yellow" href="unblock_user.php?u=<?=urlencode($u['username'])?>">Unblock</a>
              <?php else: ?>
                <a class="btn red" href="block_user.php?u=<?=urlencode($u['username'])?>" onclick="return confirm('Block this user?')">Block</a>
              <?php endif; ?>
...
if(isset($_GET['csv'])){
    ...
    fputcsv($f,['time','event','username','real_ip','virtual_ip','platform','gui_version','download_bytes','upload_bytes']);
    foreach($rows as $r){
        fputcsv($f,[$r['event_time'],$r['event_type'],$r['username'],$r['real_ip'],$r['virtual_ip'],$r['platform'],$r['gui_version'],$r['rx_bytes'],$r['tx_bytes']]);
    }
    exit;
}
...
<div class="card">
  <div class="toolbar">
    <div>
      ...
      <div class="small">Shows time, username, IP, GUI version, and usage from disconnect events.</div>
    </div>
  ...
  <div class="table-wrap">
    <table>
      <tr>
        <th>Time</th><th>Event</th><th>User</th><th>IP</th><th>GUI Version</th><th>Platform</th><th>Download</th><th>Upload</th>
      </tr>
      <?php if(!$rows): ?>
        <tr><td colspan="8" class="empty">No log rows yet.</td></tr>
      <?php else: foreach($rows as $r): ?>
        <tr>
          <td><?=esc($r['event_time'])?></td>
          <td><span class="badge <?= $r['event_type']==='disconnect' ? 'yellow' : 'green' ?>"><?=esc($r['event_type'])?></span></td>
          <td><?=esc($r['username'] ?: $r['common_name'])?></td>
          <td class="row-stack"><span><?=esc($r['real_ip'])?></span><span class="small"><?=esc($r['virtual_ip'])?></span></td>
          <td class="small"><?=esc($r['gui_version'] ?: '-')?></td>
          <td><?=esc($r['platform'] ?: '-')?></td>
          <td><?=esc(human_bytes($r['rx_bytes']))?></td>
          <td><?=esc(human_bytes($r['tx_bytes']))?></td>
        </tr>
      <?php endforeach; endif; ?>
    </table>
  </div>
