...
apt-get update
apt-get install -y openvpn easy-rsa apache2 php libapache2-mod-php php-sqlite3 php-cli sqlite3 curl openssl ca-certificates acl netcat-openbsd sudo
...
CREATE TABLE IF NOT EXISTS oc_connection_events (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  event_type TEXT NOT NULL,
  event_time TEXT DEFAULT CURRENT_TIMESTAMP,
  username TEXT,
...
case "$cmd" in
  ...
  block)
    ...
    /usr/local/bin/occtl-disconnect-user "$user" >/dev/null 2>&1 || true
...
function parse_status_lines($lines){
    $rows=[];
...
function parse_management_status($port){
    $out = shell_exec("printf 'status 3\nquit\n' | nc -N 127.0.0.1 ".intval($port)." 2>/dev/null");
    if(!$out) return [];
    $lines = preg_split("/\r?\n/", trim($out));
    return parse_status_lines($lines ?: []);
}
...
function active_clients(){
    $rows=[];
    foreach([7505=>'UDP',7506=>'TCP'] as $port=>$source){
        foreach(parse_management_status($port) as $r){
            $r['source']=$source;
...
function oc_active_sessions(){
    $json = shell_exec('sudo /usr/local/bin/occtl-json-show-users 2>/dev/null');
    if(!$json) return [];
    ...
    foreach($data as $item){
        ...
        $user = $item['Username'] ?? $item['username'] ?? $item['User'] ?? '';
        $ip = $item['IP'] ?? $item['ip'] ?? $item['Remote IP'] ?? $item['remote_ip'] ?? '';
        $vpn = $item['VPN IP'] ?? $item['vpn_ip'] ?? $item['Device IP'] ?? $item['device_ip'] ?? '';
        $since = $item['Since'] ?? $item['since'] ?? $item['Connected at'] ?? $item['connected_at'] ?? '';
        $agent = $item['User-Agent'] ?? $item['user_agent'] ?? $item['Device'] ?? $item['device'] ?? '';
        $id = $item['ID'] ?? $item['id'] ?? '';
        $rows[]=['username'=>$user,'real_ip'=>$ip,'vpn_ip'=>$vpn,'connected_since'=>$since,'user_agent'=>$agent,'session_id'=>$id];
    }
    return $rows;
}
function oc_latest_logs($limit=200){
    $st=db()->prepare("SELECT * FROM oc_connection_events WHERE event_type='connect' ORDER BY id DESC LIMIT :l");
    $st->bindValue(':l',(int)$limit,SQLITE3_INTEGER);
    $r=$st->execute();
    $rows=[]; while($row=$r->fetchArray(SQLITE3_ASSOC)) $rows[]=$row;
    return $rows;
}
function oc_last_agent_for_user($u){
    $st=db()->prepare("SELECT user_agent FROM oc_connection_events WHERE username=:u AND COALESCE(user_agent,'')<>'' ORDER BY id DESC LIMIT 1");
    $st->bindValue(':u',$u,SQLITE3_TEXT);
    $r=$st->execute();
    $row=$r?$r->fetchArray(SQLITE3_ASSOC):false;
...
<div class="grid">
  ...
  <div class="card"><div class="muted">OpenVPN max capacity</div><div class="kpi"><?=$stats['ovpn_capacity']?></div></div>
</div>
...
cat >/usr/local/bin/occtl-json-show-users <<'OCJSON'
#!/usr/bin/env bash
exec /usr/bin/occtl --json show users
OCJSON
chmod 755 /usr/local/bin/occtl-json-show-users

cat >/usr/local/bin/occtl-disconnect-user <<'OCDISC'
#!/usr/bin/env bash
set -euo pipefail
U="${1:-}"
[[ -n "$U" ]] || exit 1
exec /usr/bin/occtl disconnect user "$U"
OCDISC
chmod 755 /usr/local/bin/occtl-disconnect-user
...
$event = envv('REASON') === 'disconnect' ? 'disconnect' : 'connect';
$stmt = $db->prepare('INSERT INTO oc_connection_events(event_type,username,real_ip,vpn_ip,device,user_agent,session_id,duration,rx_bytes,tx_bytes,raw_env) VALUES (:e,:u,:rip,:vip,:d,:ua,:sid,:dur,:rx,:tx,:raw)');
$stmt->bindValue(':e',$event,SQLITE3_TEXT);
$stmt->bindValue(':u',envv('USERNAME'),SQLITE3_TEXT);
$stmt->bindValue(':rip',envv('IP_REAL'),SQLITE3_TEXT);
$stmt->bindValue(':vip',envv('IP_REMOTE'),SQLITE3_TEXT);
$stmt->bindValue(':d',envv('DEVICE'),SQLITE3_TEXT);
$stmt->bindValue(':ua',envv('USER_AGENT'),SQLITE3_TEXT);
$stmt->bindValue(':sid',envv('ID'),SQLITE3_TEXT);
$stmt->bindValue(':dur',(int)(envv('STATS_DURATION')?:0),SQLITE3_INTEGER);
$stmt->bindValue(':rx',(int)(envv('STATS_BYTES_IN')?:0),SQLITE3_INTEGER);
$stmt->bindValue(':tx',(int)(envv('STATS_BYTES_OUT')?:0),SQLITE3_INTEGER);
$stmt->bindValue(':raw',json_encode($_SERVER, JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE),SQLITE3_TEXT);
$stmt->execute();
exit(0);
?>
OCPHP
...
cat >/etc/sudoers.d/ovpn-admin-occtl <<'SUDOERS'
www-data ALL=(root) NOPASSWD: /usr/local/bin/occtl-json-show-users, /usr/local/bin/occtl-disconnect-user
SUDOERS
...
<div class="card" style="margin-top:18px">
  <div class="toolbar">
    <div>
      ...
      <div class="small">Live sessions from occtl JSON output.</div>
    </div>
    <span class="badge green"><?=count($sessions)?> active</span>
  </div>
  <div class="table-wrap">
    <table style="min-width:860px">
      <tr><th>User</th><th>IP</th><th>VPN IP</th><th>Connected</th><th>Client</th></tr>
      <?php if(!$sessions): ?>
      ...
      <?php else: foreach($sessions as $s): ?>
        <tr>
          ...
          <td class="small"><?=esc($s['user_agent'] ?: oc_last_agent_for_user($s['username']) ?: '-')?></td>
        </tr>
      <?php endforeach; endif; ?>
    </table>
  </div>
...
<div class="card" style="margin-top:18px">
  <div class="toolbar">
    <div>
      <h2 class="section-title" style="margin-bottom:6px">OpenConnect connect logs</h2>
      <div class="small">Only connect logs are shown here.</div>
    </div>
  </div>
  <div class="table-wrap">
    <table style="min-width:860px">
      <tr><th>Time</th><th>User</th><th>IP</th><th>VPN IP</th><th>Client</th></tr>
      <?php if(!$logs): ?>
