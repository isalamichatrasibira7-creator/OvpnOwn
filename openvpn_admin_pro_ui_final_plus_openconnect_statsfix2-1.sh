...
apt-get update
apt-get install -y openvpn easy-rsa apache2 php php-cli php-fpm libapache2-mod-php php-sqlite3 sqlite3 curl openssl ca-certificates acl netcat-openbsd python3 ocserv gnutls-bin

command -v php >/dev/null 2>&1 || { echo "PHP CLI not installed"; exit 1; }
...
<div class="grid">
  ...
  <div class="card"><div class="muted">OpenVPN max capacity</div><div class="kpi">100000</div></div>
</div>
...
touch /etc/ocserv/ocpasswd.allowed
chmod 600 /etc/ocserv/ocpasswd.allowed

cat >/usr/local/bin/ocserv-sync-users.sh <<'SYNC'
#!/usr/bin/env bash
set -euo pipefail
DB="/var/www/html/ovpn-admin/data/ovpn.sqlite"
PASSFILE="/etc/ocserv/ocpasswd.allowed"
TMP="$(mktemp)"
touch "$PASSFILE"
chmod 600 "$PASSFILE"
sqlite3 -separator $'\t' "$DB" "SELECT username, notes, password_hash, blocked FROM users ORDER BY id ASC;" | while IFS=$'\t' read -r user notes hash blocked; do
  [[ -n "${user:-}" ]] || continue
  if [[ "${blocked:-0}" == "1" ]]; then
    ocpasswd -c "$PASSFILE" -d "$user" >/dev/null 2>&1 || true
    continue
  fi
  # use notes column to store plain password for OpenConnect sync
  plain="${notes#OC_PASS:}"
  if [[ "${plain}" == "${notes}" || -z "${plain}" ]]; then
    continue
  fi
  printf '%s\n%s\n' "$plain" "$plain" | ocpasswd -c "$PASSFILE" "$user" >/dev/null 2>&1 || true
done
rm -f "$TMP"
SYNC
chmod +x /usr/local/bin/ocserv-sync-users.sh

python3 - <<'PY'
from pathlib import Path
p = Path('/usr/local/bin/ovpn-user-manage.sh')
s = p.read_text()
s = s.replace("INSERT INTO users(username,password_hash,blocked,notes) VALUES('$(sql_escape \"$user\")','$(sql_escape \"$hash\")',0,'');",
              "INSERT INTO users(username,password_hash,blocked,notes) VALUES('$(sql_escape \"$user\")','$(sql_escape \"$hash\")',0,'OC_PASS:$(sql_escape \"$pass\")');")
s = s.replace("UPDATE users SET password_hash='$(sql_escape \"$hash\")', updated_at=CURRENT_TIMESTAMP WHERE username='$(sql_escape \"$user\")';",
              "UPDATE users SET password_hash='$(sql_escape \"$hash\")', notes='OC_PASS:$(sql_escape \"$pass\")', updated_at=CURRENT_TIMESTAMP WHERE username='$(sql_escape \"$user\")';")
for old in [
    'echo "User added: $user"',
    'echo "User updated: $user"',
    'echo "User deleted: $user"',
    ...
    s = s.replace(old, '/usr/local/bin/ocserv-sync-users.sh >/dev/null 2>&1 || true\n    ' + old)
p.write_text(s)
PY

# seed default user's OpenConnect password in notes
sqlite3 "$DB_FILE" "UPDATE users SET notes='OC_PASS:${DEFAULT_USER_PASS}' WHERE username='${DEFAULT_USER}';"
/usr/local/bin/ocserv-sync-users.sh >/dev/null 2>&1 || true

cat >/etc/ocserv/ocserv.conf <<EOF
auth = "plain[passwd=/etc/ocserv/ocpasswd.allowed]"
tcp-port = 4443
udp-port = 4443
run-as-user = nobody
run-as-group = daemon
...
tunnel-all-dns = true
route = default
no-route = 10.20.30.0/255.255.255.0
keepalive = 32400
dpd = 90
mobile-dpd = 1800
...
function oc_sessions(){
    $out=shell_exec("occtl show users 2>/dev/null");
    if(!$out) return [];
    $sessions=[];
    $current=[];
...
<div class="card" style="margin-top:18px">
  <div class="toolbar">
    <div>
      ...
      <div class="small">Shared users from the same panel database can log in here too.</div>
    </div>
...
echo "[17/17] Applying stats/live panel fixes..."
apt-get update >/dev/null 2>&1 || true
apt-get install -y sudo python3 php-cli >/dev/null 2>&1 || true

cat >/usr/local/bin/occtl-list-users <<'OCLIST'
#!/usr/bin/env bash
set -euo pipefail
OUT="$(occtl show users 2>/dev/null || true)"
[[ -n "$OUT" ]] || exit 0
awk 'BEGIN{IGNORECASE=1; FS=":"; OFS="\t"}
function trim(x){gsub(/^[ \t]+|[ \t]+$/,"",x); return x}
/^id:/ {if (have){print user,ip,vpn,conn,agent}; have=1; user=ip=vpn=conn=agent="-"; next}
/^(username|user):/ {sub(/^[^:]*:/,""); user=trim($0); next}
/^(ip|remote ip|remote_ip):/ {sub(/^[^:]*:/,""); ip=trim($0); next}
/^(device ip|vpn ip|device_ip|vpn_ip):/ {sub(/^[^:]*:/,""); vpn=trim($0); next}
/^(conn time|connected at|connected_at):/ {sub(/^[^:]*:/,""); conn=trim($0); next}
/^(user-agent|user agent|device):/ {sub(/^[^:]*:/,""); agent=trim($0); next}
END{if (have){print user,ip,vpn,conn,agent}}' <<< "$OUT"
OCLIST
chmod +x /usr/local/bin/occtl-list-users

echo 'www-data ALL=(root) NOPASSWD: /usr/local/bin/occtl-list-users' >/etc/sudoers.d/ovpn-admin-occtl
chmod 440 /etc/sudoers.d/ovpn-admin-occtl

cat >/usr/local/bin/ocserv-log-event.php <<'OCPHP'
#!/usr/bin/env php
<?php
$db = new SQLite3('/var/www/html/ovpn-admin/data/ovpn.sqlite');
$db->busyTimeout(5000);
function ev($keys){ foreach($keys as $k){ $v=getenv($k); if($v!==false && $v!=='') return (string)$v; } return ''; }
$stype = ev(['REASON','SCRIPT_TYPE','script_type']);
$event = (stripos($stype,'disconnect')!==false) ? 'oc_disconnect' : 'oc_connect';
$username = ev(['USERNAME','USER','username']);
$real = ev(['IP_REAL','REMOTE_HOST','trusted_ip']);
$virt = ev(['IP_REMOTE','INTERNAL_IP4_ADDRESS','ifconfig_pool_remote_ip']);
$agent = ev(['USER_AGENT','DEVICE','DEVICE_TYPE','PLATFORM']);
$duration = (int)ev(['STATS_DURATION','time_duration']);
$rx = (int)ev(['STATS_BYTES_IN','bytes_received']);
$tx = (int)ev(['STATS_BYTES_OUT','bytes_sent']);
$raw = json_encode($_SERVER, JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
$stmt = $db->prepare('INSERT INTO connection_events(event_type,username,common_name,real_ip,virtual_ip,gui_version,time_duration,rx_bytes,tx_bytes,raw_peer_info,app_hint) VALUES (:e,:u,:c,:r,:v,:g,:d,:rx,:tx,:raw,:app)');
$stmt->bindValue(':e',$event,SQLITE3_TEXT);
$stmt->bindValue(':u',$username,SQLITE3_TEXT);
$stmt->bindValue(':c',$username,SQLITE3_TEXT);
$stmt->bindValue(':r',$real,SQLITE3_TEXT);
$stmt->bindValue(':v',$virt,SQLITE3_TEXT);
$stmt->bindValue(':g',$agent,SQLITE3_TEXT);
$stmt->bindValue(':d',$duration,SQLITE3_INTEGER);
$stmt->bindValue(':rx',$rx,SQLITE3_INTEGER);
$stmt->bindValue(':tx',$tx,SQLITE3_INTEGER);
$stmt->bindValue(':raw',$raw,SQLITE3_TEXT);
$stmt->bindValue(':app',$agent,SQLITE3_TEXT);
$stmt->execute();
OCPHP
chmod +x /usr/local/bin/ocserv-log-event.php

python3 - <<'PY2'
from pathlib import Path
p = Path('/etc/ocserv/ocserv.conf')
s = p.read_text() if p.exists() else ''
for line in [
    'connect-script = /usr/local/bin/ocserv-log-event.php',
    'disconnect-script = /usr/local/bin/ocserv-log-event.php',
]:
    if line not in s:
        s += '\n' + line + '\n'
p.write_text(s)
...
cat >/var/www/html/ovpn-admin/config.php <<'PHP'
<?php
session_start();
date_default_timezone_set('UTC');
define('DB_PATH', __DIR__ . '/data/ovpn.sqlite');
...
function users_all(){ $res=db()->query("SELECT id,username,blocked,created_at,updated_at FROM users ORDER BY id DESC"); $rows=[]; while($row=$res->fetchArray(SQLITE3_ASSOC)) $rows[]=$row; return $rows; }
...
function latest_logs($limit=300,$search=''){ $where="(event_type='connect' OR event_type='oc_connect')"; if($search!==''){ $st=db()->prepare("SELECT * FROM connection_events WHERE $where AND (username LIKE :s OR real_ip LIKE :s OR gui_version LIKE :s OR app_hint LIKE :s) ORDER BY id DESC LIMIT :l"); $st->bindValue(':s','%'.$search.'%',SQLITE3_TEXT); $st->bindValue(':l',(int)$limit,SQLITE3_INTEGER); $r=$st->execute(); } else { $st=db()->prepare("SELECT * FROM connection_events WHERE $where ORDER BY id DESC LIMIT :l"); $st->bindValue(':l',(int)$limit,SQLITE3_INTEGER); $r=$st->execute(); } $rows=[]; while($row=$r->fetchArray(SQLITE3_ASSOC)) $rows[]=$row; return $rows; }
function parse_status_lines($lines,$source=''){ $clients=[]; $routes=[]; foreach($lines as $line){ $line=trim((string)$line); if($line==='') continue; if(strpos($line,'CLIENT_LIST')===0){ $p=preg_split('/[\t,]/',$line); $common=$p[1]??''; $real=$p[2]??''; $key=$common.'|'.$real; $clients[$key]=['common_name'=>$common,'real_address'=>$real,'bytes_received'=>(int)($p[3]??0),'bytes_sent'=>(int)($p[4]??0),'connected_since'=>$p[5]??'','virtual_address'=>'','username'=>$p[8]??($p[7]??$common),'source'=>$source]; } elseif(strpos($line,'ROUTING_TABLE')===0){ $p=preg_split('/[\t,]/',$line); $routes[]=['virtual_address'=>$p[1]??'','common_name'=>$p[2]??'','real_address'=>$p[3]??'','username'=>$p[5]??'']; } } foreach($routes as $rt){ foreach($clients as $key=>$cl){ if(($rt['common_name']!=='' && $rt['common_name']===$cl['common_name']) || ($rt['real_address']!=='' && $rt['real_address']===$cl['real_address'])){ if($rt['virtual_address']!=='') $clients[$key]['virtual_address']=$rt['virtual_address']; if(($clients[$key]['username']??'')==='' && $rt['username']!=='') $clients[$key]['username']=$rt['username']; } } } return array_values($clients); }
function parse_status_file($file,$source=''){ if(!is_file($file)||!is_readable($file)) return []; $lines=@file($file, FILE_IGNORE_NEW_LINES|FILE_SKIP_EMPTY_LINES); if(!$lines) return []; return parse_status_lines($lines,$source); }
function parse_management_status($port,$source=''){ $raw=@shell_exec("printf 'status 3\nquit\n' | nc -N 127.0.0.1 ".intval($port)." 2>/dev/null"); if(!$raw) return []; return parse_status_lines(preg_split('/\r?\n/',trim($raw)),$source); }
...
function active_from_events(){ $sql="SELECT e1.username,e1.common_name,e1.real_ip AS real_address,e1.virtual_ip AS virtual_address,e1.event_time AS connected_since,e1.gui_version FROM connection_events e1 INNER JOIN (SELECT COALESCE(username,'') AS u, COALESCE(real_ip,'') AS ip, MAX(id) AS max_id FROM connection_events WHERE event_type='connect' AND COALESCE(username,'')<>'' GROUP BY COALESCE(username,''), COALESCE(real_ip,'')) latest ON latest.max_id=e1.id WHERE e1.event_type='connect'"; $res=db()->query($sql); $rows=[]; while($row=$res->fetchArray(SQLITE3_ASSOC)) $rows[]=['common_name'=>$row['common_name']?:$row['username'],'real_address'=>$row['real_address']?:'-','bytes_received'=>0,'bytes_sent'=>0,'connected_since'=>$row['connected_since']?:'','virtual_address'=>$row['virtual_address']?:'-','username'=>$row['username']?:($row['common_name']?:''),'source'=>'EVENT','gui_version'=>$row['gui_version']??'']; return $rows; }
function active_clients(){ $all=[]; foreach([[7505,'UDP'],[7506,'TCP']] as $pair){ foreach(parse_management_status($pair[0],$pair[1]) as $r){ $u=($r['username']??'')?:($r['common_name']??''); $r['gui_version']=last_gui_for_user($u); $all[$u.'|'.$r['real_address'].'|'.$pair[1]]=$r; } } if(!$all){ foreach(['/var/log/openvpn/openvpn-status-udp.log'=>'UDP','/var/log/openvpn/openvpn-status-tcp.log'=>'TCP'] as $file=>$source){ foreach(parse_status_file($file,$source) as $r){ $u=($r['username']??'')?:($r['common_name']??''); $r['gui_version']=last_gui_for_user($u); $all[$u.'|'.$r['real_address'].'|'.$source]=$r; } } } if(!$all) return active_from_events(); return array_values($all); }
function dashboard_stats(){ $active=active_clients(); return ['total_users'=>(int)db()->querySingle('SELECT COUNT(*) FROM users'),'active_connections'=>count($active),'active_users'=>count(array_unique(array_map(fn($x)=>($x['username']?:$x['common_name']),$active))),'today_connects'=>(int)db()->querySingle("SELECT COUNT(*) FROM connection_events WHERE (event_type='connect' OR event_type='oc_connect') AND date(event_time)=date('now')"),'gui_rows'=>(int)db()->querySingle("SELECT COUNT(*) FROM connection_events WHERE (event_type='connect' OR event_type='oc_connect') AND COALESCE(gui_version,'')<>''"),'ovpn_capacity'=>100000,'oc_capacity'=>100000]; }
function oc_sessions_from_occtl(){ $raw=@shell_exec("sudo /usr/local/bin/occtl-list-users 2>/dev/null"); if(!$raw) return []; $sessions=[]; foreach(preg_split('/\r?\n/', trim($raw)) as $line){ if(trim($line)==='') continue; $p=explode("\t",$line); $sessions[]=['username'=>$p[0]??'-','ip'=>$p[1]??'-','vpn_ip'=>$p[2]??'-','connected'=>$p[3]??'-','agent'=>$p[4]??'-']; } return $sessions; }
function oc_sessions_from_events(){ $sql="SELECT e1.username,e1.real_ip,e1.virtual_ip,e1.event_time,e1.gui_version,e1.app_hint FROM connection_events e1 INNER JOIN (SELECT COALESCE(username,'') AS u, COALESCE(real_ip,'') AS ip, MAX(id) AS max_id FROM connection_events WHERE (event_type='oc_connect' OR event_type='oc_disconnect') AND COALESCE(username,'')<>'' GROUP BY COALESCE(username,''), COALESCE(real_ip,'')) latest ON latest.max_id=e1.id WHERE e1.event_type='oc_connect'"; $res=db()->query($sql); $rows=[]; while($row=$res->fetchArray(SQLITE3_ASSOC)) $rows[]=['username'=>$row['username']?:'-','ip'=>$row['real_ip']?:'-','vpn_ip'=>$row['virtual_ip']?:'-','connected'=>$row['event_time']?:'-','agent'=>$row['gui_version']?:($row['app_hint']?:'-')]; return $rows; }
function oc_sessions(){ $s=oc_sessions_from_occtl(); return $s ?: oc_sessions_from_events(); }
function render_header($title='OpenVPN Admin'){ ?>
<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="refresh" content="15"><title><?=esc($title)?></title><link rel="stylesheet" href="style.css"></head><body><div class="shell"><header class="site-header"><div class="brand-wrap"><button class="menu-btn" type="button" onclick="document.body.classList.toggle('menu-open')">☰</button><div><div class="brand">OpenVPN Admin</div><div class="sub">Logged in as <?=esc($_SESSION['admin_user'] ?? '')?></div></div></div><a class="refresh-btn" href="<?=esc(basename($_SERVER['PHP_SELF']).(!empty($_SERVER['QUERY_STRING'])?'?'.$_SERVER['QUERY_STRING']:''))?>">Refresh</a></header><div class="layout"><aside class="sidebar"><nav class="menu"><a href="index.php">Dashboard</a><a href="users.php">Users</a><a href="new_user.php">New User</a><a href="logs.php">Connection Logs</a><a href="openconnect.php">OpenConnect</a><a href="change_password.php">Change Admin Password</a><a href="logout.php">Logout</a></nav></aside><main class="content">
<?php }
function render_footer(){ ?>
  </main></div></div><script>document.addEventListener('click', function(e){ if (e.target.matches('.overlay-close')) document.body.classList.remove('menu-open'); });</script></body></html>
<?php }
PHP

cat >/var/www/html/ovpn-admin/index.php <<'PHP'
<?php require __DIR__.'/config.php'; require_login(); $stats=dashboard_stats(); $active=active_clients(); render_header('Dashboard'); ?>
<div class="grid"><div class="card"><div class="muted">Total users</div><div class="kpi"><?=$stats['total_users']?></div></div><div class="card"><div class="muted">OpenVPN max capacity</div><div class="kpi"><?=$stats['ovpn_capacity']?></div></div><div class="card"><div class="muted">Active connections</div><div class="kpi"><?=$stats['active_connections']?></div></div><div class="card"><div class="muted">Active users</div><div class="kpi"><?=$stats['active_users']?></div></div><div class="card"><div class="muted">Today connects</div><div class="kpi"><?=$stats['today_connects']?></div></div><div class="card"><div class="muted">Rows with GUI version</div><div class="kpi"><?=$stats['gui_rows']?></div></div></div>
<div class="card" style="margin-top:18px"><div class="toolbar"><div><h2 class="section-title" style="margin-bottom:6px">Active connected devices</h2><div class="small">Refresh the page to update the live device table.</div></div><span class="badge green"><?=$stats['active_connections']?> active now</span></div><div class="table-wrap"><table><tr><th>User</th><th>GUI Version</th><th>Real IP</th><th>Virtual IP</th><th>Since</th><th>Download</th><th>Upload</th><th>Action</th></tr><?php if(!$active): ?><tr><td colspan="8" class="empty">No active devices right now. Connect a device and refresh after a few seconds.</td></tr><?php else: foreach($active as $c): $u=($c['username'] ?: $c['common_name']); $isBlocked=(int)db()->querySingle("SELECT blocked FROM users WHERE username='".SQLite3::escapeString($u)."' LIMIT 1"); ?><tr><td><strong><?=esc($u)?></strong><div class="small"><?=esc($c['source'] ?? '')?></div></td><td class="small"><?=esc($c['gui_version'] ?: last_gui_for_user($u) ?: '-')?></td><td><?=esc($c['real_address'])?></td><td><?=esc($c['virtual_address'] ?: '-')?></td><td><?=esc($c['connected_since'])?></td><td><?=esc(human_bytes($c['bytes_received'] ?? 0))?></td><td><?=esc(human_bytes($c['bytes_sent'] ?? 0))?></td><td><div class="actions"><?php if($isBlocked===1): ?><a class="btn yellow" href="unblock_user.php?u=<?=urlencode($u)?>">Unblock</a><?php else: ?><a class="btn red" href="block_user.php?u=<?=urlencode($u)?>" onclick="return confirm('Block this user?')">Block</a><?php endif; ?></div></td></tr><?php endforeach; endif; ?></table></div></div>
<?php render_footer(); ?>
PHP

cat >/var/www/html/ovpn-admin/logs.php <<'PHP'
<?php require __DIR__.'/config.php'; require_login(); $q=trim($_GET['q']??''); $rows=latest_logs(300,$q); if(isset($_GET['csv'])){ header('Content-Type:text/csv'); header('Content-Disposition: attachment; filename="vpn-connect-logs.csv"'); $f=fopen('php://output','w'); fputcsv($f,['time','protocol','username','ip','gui_version']); foreach($rows as $r){ $proto = strpos($r['event_type'],'oc_')===0 ? 'OpenConnect' : 'OpenVPN'; fputcsv($f,[$r['event_time'],$proto,$r['username'] ?: $r['common_name'],$r['real_ip'],$r['gui_version'] ?: $r['app_hint']]); } exit; } render_header('Connection Logs'); ?>
<div class="card"><div class="toolbar"><div><h2 class="section-title" style="margin-bottom:6px">Connection logs</h2><div class="small">Connected logs for both OpenVPN and OpenConnect are shown here.</div></div><a class="btn" href="logs.php?csv=1<?= $q!=='' ? '&q='.urlencode($q) : '' ?>">Export CSV</a></div><form method="get" class="actions" style="margin-bottom:14px"><input name="q" placeholder="Search username, IP, GUI version" value="<?=esc($q)?>"><button class="btn gray" type="submit">Search</button></form><div class="table-wrap"><table style="min-width:880px"><tr><th>Time</th><th>Protocol</th><th>User</th><th>IP</th><th>GUI / Agent</th></tr><?php if(!$rows): ?><tr><td colspan="5" class="empty">No connected logs yet.</td></tr><?php else: foreach($rows as $r): $proto = strpos($r['event_type'],'oc_')===0 ? 'OpenConnect' : 'OpenVPN'; ?><tr><td><?=esc($r['event_time'])?></td><td><?=esc($proto)?></td><td><?=esc($r['username'] ?: $r['common_name'])?></td><td><?=esc($r['real_ip'])?></td><td class="small"><?=esc($r['gui_version'] ?: $r['app_hint'] ?: '-')?></td></tr><?php endforeach; endif; ?></table></div></div>
<?php render_footer(); ?>
PHP

cat >/var/www/html/ovpn-admin/openconnect.php <<'PHP'
<?php require __DIR__.'/config.php'; require_login(); $sessions = oc_sessions(); $logrows = db()->query("SELECT * FROM connection_events WHERE event_type='oc_connect' ORDER BY id DESC LIMIT 100"); $logs=[]; while($row=$logrows->fetchArray(SQLITE3_ASSOC)) $logs[]=$row; render_header('OpenConnect'); ?>
<div class="grid"><div class="card"><div class="muted">OpenConnect max capacity</div><div class="kpi">100000</div></div><div class="card"><div class="muted">OpenConnect active now</div><div class="kpi"><?=count($sessions)?></div></div></div>
<div class="card" style="margin-top:18px"><div class="toolbar"><div><h2 class="section-title" style="margin-bottom:6px">OpenConnect active sessions</h2><div class="small">Shared users from the same panel database can log in here too.</div></div><span class="badge green"><?=count($sessions)?> active</span></div><div class="table-wrap"><table style="min-width:860px"><tr><th>User</th><th>IP</th><th>VPN IP</th><th>Connected</th><th>Agent</th></tr><?php if(!$sessions): ?><tr><td colspan="5" class="empty">No active OpenConnect sessions.</td></tr><?php else: foreach($sessions as $s): ?><tr><td><?=esc($s['username'])?></td><td><?=esc($s['ip'])?></td><td><?=esc($s['vpn_ip'])?></td><td><?=esc($s['connected'])?></td><td class="small"><?=esc($s['agent'])?></td></tr><?php endforeach; endif; ?></table></div><div class="small" style="margin-top:12px">OpenConnect URL: https://<?=esc($_SERVER['SERVER_ADDR'] ?: $_SERVER['SERVER_NAME'])?>:4443</div></div>
<div class="card" style="margin-top:18px"><div class="toolbar"><div><h2 class="section-title" style="margin-bottom:6px">OpenConnect logs</h2><div class="small">Latest successful OpenConnect connect events.</div></div></div><div class="table-wrap"><table style="min-width:860px"><tr><th>Time</th><th>User</th><th>IP</th><th>VPN IP</th><th>Agent</th></tr><?php if(!$logs): ?><tr><td colspan="5" class="empty">No OpenConnect logs yet.</td></tr><?php else: foreach($logs as $r): ?><tr><td><?=esc($r['event_time'])?></td><td><?=esc($r['username'] ?: $r['common_name'])?></td><td><?=esc($r['real_ip'])?></td><td><?=esc($r['virtual_ip'])?></td><td class="small"><?=esc($r['gui_version'] ?: $r['app_hint'] ?: '-')?></td></tr><?php endforeach; endif; ?></table></div></div>
<?php render_footer(); ?>
PHP
...
echo "Stats/live panel fixes applied."
