...
function oc_sessions(){
    ...
    foreach($data as $row){
        ...
        $sessions[]=[
            ...
            'ip'=>$row['IP'] ?? $row['ip'] ?? $row['Remote IP'] ?? $row['remote_ip'] ?? '-',
            'vpn_ip'=>$row['VPN IP'] ?? $row['vpn_ip'] ?? $row['Device IP'] ?? $row['device_ip'] ?? '-',
            'connected'=>$row['Connected at'] ?? $row['Since'] ?? $row['connected_at'] ?? $row['since'] ?? '-',
            'user_agent'=>$row['User-Agent'] ?? $row['user_agent'] ?? $row['Device'] ?? $row['device'] ?? '-',
        ];
    }
    return $sessions;
}
    ...
    <table style="min-width:860px">
      ...
      <?php else: foreach($sessions as $s): ?>
        <tr>
          ...
          <td><?=esc($s['ip'] ?? '-')?></td>
          <td><?=esc($s['vpn_ip'] ?? '-')?></td>
          <td><?=esc($s['connected'] ?? '-')?></td>
          <td class="small"><?=esc($s['user_agent'] ?? '-')?></td>
        </tr>
      <?php endforeach; endif; ?>
    </table>
  </div>
