...
systemctl stop openvpn-server@server-udp openvpn-server@server-tcp apache2 ovpn-iptables.service ocserv 2>/dev/null || true
systemctl disable openvpn-server@server-udp openvpn-server@server-tcp ovpn-iptables.service ocserv 2>/dev/null || true
rm -rf "$APP_DIR" "$PKI_DIR" "$OCSERV_DIR"
rm -f "$OVPN_DIR/server-udp.conf" "$OVPN_DIR/server-tcp.conf"
rm -f "$BIN_DIR/ovpn-auth.php" "$BIN_DIR/ovpn-log-event.php" "$BIN_DIR/ovpn-make-profile.sh" "$BIN_DIR/ovpn-user-manage.sh" "$BIN_DIR/ovpn-iptables-apply.sh" "$BIN_DIR/ocserv-sync-users.sh" "$BIN_DIR/ocserv-kill-user.sh" "$BIN_DIR/ocserv-pam-auth.sh" "$BIN_DIR/ocserv-connect.sh" "$BIN_DIR/ocserv-disconnect.sh"
...
apt-get update
apt-get install -y openvpn easy-rsa apache2 php libapache2-mod-php php-sqlite3 php-cli sqlite3 curl openssl ca-certificates acl netcat-openbsd ocserv gnutls-bin ssl-cert

echo "[3/12] Creating directories..."
mkdir -p "$APP_DIR" "$DATA_DIR" "$DOWNLOAD_DIR" "$PKI_DIR" "$OVPN_DIR" "$LOG_DIR" "$BIN_DIR" "$OCSERV_DIR"
...
case "$cmd" in
  ...
  delete)
    ...
    /usr/local/bin/ocserv-kill-user.sh "$user" >/dev/null 2>&1 || true
    /usr/local/bin/ocserv-sync-users.sh >/dev/null 2>&1 || true
...
cat >"$BIN_DIR/ocserv-sync-users.sh" <<'OCSYNC'
#!/usr/bin/env bash
set -euo pipefail
DB="/var/www/html/ovpn-admin/data/ovpn.sqlite"
...
: > "$TMP"
if [[ -f "$DB" ]]; then
  while IFS=$'\t' read -r username blocked; do
    [[ -n "${username:-}" ]] || continue
    if [[ "${blocked:-0}" == "1" ]]; then
      continue
    fi
    echo "$username" >> "$TMP"
  done < <(sqlite3 -tabs "$DB" "SELECT username, blocked FROM users ORDER BY username;")
fi
install -m 600 -o root -g root "$TMP" "$PASSFILE"
rm -f "$TMP"
OCSYNC
chmod +x "$BIN_DIR/ocserv-sync-users.sh"

cat >"$BIN_DIR/ocserv-kill-user.sh" <<'OCKILL'
#!/usr/bin/env bash
set -euo pipefail
USER_NAME="${1:-}"
[[ -n "$USER_NAME" ]] || exit 0
occtl disconnect user "$USER_NAME" >/dev/null 2>&1 || true
OCKILL
chmod +x "$BIN_DIR/ocserv-kill-user.sh"

make-ssl-cert generate-default-snakeoil --force-overwrite >/dev/null 2>&1 || true
SERVER_IP_CERT="${SERVER_ADDR}"
mkdir -p "$OCSERV_DIR"
openssl req -x509 -nodes -newkey rsa:2048   -keyout "$OCSERV_DIR/server-key.pem"   -out "$OCSERV_DIR/server-cert.pem"   -days 3650   -subj "/CN=${SERVER_IP_CERT}"   -addext "subjectAltName = IP:${SERVER_IP_CERT}" >/dev/null 2>&1
chmod 600 "$OCSERV_DIR/server-key.pem"
chmod 644 "$OCSERV_DIR/server-cert.pem"

cat >"$OCSERV_DIR/ocserv.conf" <<CONF
...
use-occtl = true
connect-script = ${BIN_DIR}/ocserv-connect.sh
disconnect-script = ${BIN_DIR}/ocserv-disconnect.sh
CONF

cat >/etc/pam.d/ocserv-panel <<'PAM'
auth required pam_exec.so expose_authtok /usr/local/bin/ocserv-pam-auth.sh
account required pam_permit.so
PAM

cat >"$BIN_DIR/ocserv-pam-auth.sh" <<'OCAUTH'
#!/usr/bin/env bash
set -euo pipefail
DB="/var/www/html/ovpn-admin/data/ovpn.sqlite"
read -r PASSWORD || PASSWORD=""
USER_NAME="${PAM_USER:-}"
[[ -n "$USER_NAME" && -n "$PASSWORD" && -f "$DB" ]] || exit 1
HASH="$(sqlite3 "$DB" "SELECT password_hash FROM users WHERE username='${USER_NAME//'/''}' AND blocked=0 LIMIT 1;" 2>/dev/null || true)"
[[ -n "$HASH" ]] || exit 1
php -r '$hash=$argv[1]; $pass=$argv[2]; exit(password_verify($pass,$hash)?0:1);' "$HASH" "$PASSWORD"
OCAUTH
chmod +x "$BIN_DIR/ocserv-pam-auth.sh"

cat >"$BIN_DIR/ocserv-connect.sh" <<'OCCONN'
#!/usr/bin/env bash
set -euo pipefail
DB="/var/www/html/ovpn-admin/data/ovpn.sqlite"
[[ -f "$DB" ]] || exit 0
USER_NAME="${USERNAME:-${USER_NAME:-}}"
VIRTUAL_IP="${IP_REAL:-${IPV4_ADDR:-}}"
REAL_IP="${IP_REMOTE:-${REMOTE_IP:-}}"
GUI="${USER_AGENT:-${DEVICE_TYPE:-OpenConnect}}"
/usr/bin/sqlite3 "$DB" "INSERT INTO connection_events(event_type,event_time,username,common_name,real_ip,virtual_ip,platform,platform_version,openvpn_version,gui_version,ssl_library,hwaddr,time_duration,rx_bytes,tx_bytes,raw_peer_info,app_hint) VALUES('connect',CURRENT_TIMESTAMP,'${USER_NAME//'/''}','${USER_NAME//'/''}','${REAL_IP//'/''}','${VIRTUAL_IP//'/''}','OpenConnect','','ocserv','${GUI//'/''}','','',0,0,0,'','${GUI//'/''}');" >/dev/null 2>&1 || true
OCCONN
chmod +x "$BIN_DIR/ocserv-connect.sh"

cat >"$BIN_DIR/ocserv-disconnect.sh" <<'OCDIS'
#!/usr/bin/env bash
exit 0
OCDIS
chmod +x "$BIN_DIR/ocserv-disconnect.sh"
...
function ocserv_active_clients(){
    $rows=[];
    $raw=shell_exec("occtl show users 2>/dev/null || true");
    if(!$raw) return $rows;
    $chunks=preg_split('/\n\s*\n/', trim($raw));
    foreach($chunks as $chunk){
        if(trim($chunk)==='') continue;
        $row=['username'=>'','real_address'=>'-','virtual_address'=>'-','connected_since'=>'','gui_version'=>'OpenConnect','source'=>'OC'];
        foreach(preg_split('/\n/', $chunk) as $line){
            $line=trim($line);
            if(preg_match('/^(?:user|username)[: ]+(.+)$/i',$line,$m)) $row['username']=trim($m[1]);
            elseif(preg_match('/^(?:ip|remote ip|remote-ip)[: ]+(.+)$/i',$line,$m)) $row['real_address']=trim($m[1]);
            elseif(preg_match('/^(?:vpn ip|ipv4)[: ]+(.+)$/i',$line,$m)) $row['virtual_address']=trim($m[1]);
            elseif(preg_match('/^(?:connected at|since|conn time|connected)[: ]+(.+)$/i',$line,$m)) $row['connected_since']=trim($m[1]);
            elseif(preg_match('/^(?:user-agent|device)[: ]+(.+)$/i',$line,$m)) $row['gui_version']=trim($m[1]);
        }
        if($row['username']!=='') $rows[]=$row;
    }
    return $rows;
}
function combined_active_connections_count(){
    return count(active_clients()) + count(ocserv_active_clients());
}
function combined_active_users_count(){
    $names=[];
    foreach(active_clients() as $r){ $u=$r['username'] ?: $r['common_name']; if($u!=='') $names[$u]=1; }
    foreach(ocserv_active_clients() as $r){ if(($r['username'] ?? '')!=='') $names[$r['username']]=1; }
    return count($names);
}
function dashboard_stats(){
    ...
    return [
        ...
        'active_connections'=>combined_active_connections_count(),
        'active_users'=>combined_active_users_count(),
...
<div class="layout">
  <aside class="sidebar">
    <nav class="menu">
      ...
      <a href="openconnect.php">OpenConnect</a>
...
<?php require __DIR__.'/config.php'; require_login(); $rows=ocserv_active_clients(); render_header('OpenConnect'); ?>
<div class="grid">
  <div class="card"><div class="muted">OpenConnect max capacity</div><div class="kpi"><?=esc(MAX_CAPACITY)?></div></div>
  <div class="card"><div class="muted">OpenConnect active now</div><div class="kpi"><?=count($rows)?></div></div>
</div>
<div class="card" style="margin-top:18px">
  <div class="toolbar">
    <div>
    ...
    <span class="badge green"><?=count($rows)?> active</span>
  </div>
  <div class="table-wrap">
    <table style="min-width:760px">
      <tr><th>User</th><th>GUI Version</th><th>Real IP</th><th>Virtual IP</th><th>Since</th></tr>
      <?php if(!$rows): ?>
        <tr><td colspan="5" class="empty">No active OpenConnect sessions right now.</td></tr>
      <?php else: foreach($rows as $r): ?>
        <tr>
          <td><strong><?=esc($r['username'])?></strong></td>
          <td class="small"><?=esc($r['gui_version'] ?: 'OpenConnect')?></td>
          <td><?=esc($r['real_address'])?></td>
          <td><?=esc($r['virtual_address'])?></td>
          <td><?=esc($r['connected_since'])?></td>
        </tr>
      <?php endforeach; endif; ?>
    </table>
  </div>
  <div class="small" style="margin-top:12px">OpenConnect URL: https://<?=esc($_SERVER['HTTP_HOST'])?>:4443</div>
</div>
<?php render_footer(); ?>
PHP
...
echo "OpenConnect URL: https://${SERVER_ADDR}:${OC_PORT}"
echo "OpenConnect max capacity shown in panel: ${MAX_CAPACITY}"
echo
echo "CLI commands:"
